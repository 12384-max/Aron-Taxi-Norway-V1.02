              ...
              <div className="bg-[#0E131F]/90 border border-emerald-500/40 rounded-3xl p-5 sm:p-6 shadow-2xl space-y-5">
                ...
                <div className="rounded-2xl overflow-hidden border border-white/10 h-72">
                  <LeafletMap
                    ...
                    driverLocation={activeTrip.driverLocation || assignedDriver?.currentLocation}
                    routeGeometry={activeTrip.routeGeometry}
                    zoom={14}
                  />
                </div>

                {/* DRIVER PROFILE & RATES CARD (IF ASSIGNED) */}
                {(assignedDriver || activeTrip.driverName) && (
                  <div className="bg-[#121826] rounded-2xl p-5 border border-[#D4AF37]/30 space-y-4 shadow-xl">
                    
                    {/* Driver Identity */}
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
                      <div className="flex items-center gap-3.5">
                        <div className="relative shrink-0">
                          <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-[#E5B83B] to-[#F3D37A] p-0.5 shadow-lg">
                            {assignedDriver?.avatarUrl ? (
                              ...
                              <div className="w-full h-full rounded-2xl bg-slate-950 flex items-center justify-center font-black text-lg text-[#E5B83B]">
                                {(assignedDriver?.name || activeTrip.driverName || 'Aron').slice(0, 2).toUpperCase()}
                              </div>
                            )}
                          </div>
                          <span className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-500 border-2 border-[#121826] flex items-center justify-center text-slate-950">
                        ...
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <h4 className="text-sm sm:text-base font-bold text-white">
                              {assignedDriver?.name || activeTrip.driverName || 'Aron VIP Sjåfør'}
                            </h4>
                            <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-[9px] font-black rounded-full uppercase">
                              Verifisert
                            </span>
                            <span className="px-2 py-0.5 bg-slate-700 text-slate-200 border border-slate-600 text-[9px] font-bold rounded-full flex items-center gap-1">
                          ...
                          <div className="flex items-center gap-2 mt-1 text-xs text-slate-300">
                            <span className="flex items-center gap-1 font-bold text-amber-400">
                              ...
                              {assignedDriver?.rating ? assignedDriver.rating.toFixed(1) : '5.0'}
                              <span className="text-slate-400 font-normal font-mono">
                                ({assignedDriver?.ratingCount || 1})
                              </span>
                            </span>
                            <span>•</span>
                            <span className="text-slate-400 font-mono text-[10px]">
                              Løyve: <strong className="text-[#E5B83B]">{activeTrip.permitNumber || assignedVehicle?.permitNumber || assignedDriver?.permitNumber || 'OS 10597'}</strong>
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        {(assignedDriver?.phone || activeTrip.driverPhone) && (
                          <>
                            <a
                              href={`tel:${assignedDriver?.phone || activeTrip.driverPhone}`}
                              className="px-4 py-2 bg-gradient-to-r from-[#D4AF37] to-[#C5A028] text-slate-950 font-black text-xs uppercase tracking-wider rounded-xl shadow-lg flex items-center gap-1.5 hover:brightness-110"
                            >
                              <Phone className="w-3.5 h-3.5" />
                              Ring
                            </a>
                            <a
                              href={`sms:${assignedDriver?.phone || activeTrip.driverPhone}`}
                              className="px-4 py-2 bg-white/10 hover:bg-white/15 text-white font-bold text-xs uppercase tracking-wider rounded-xl border border-white/10 flex items-center gap-1.5"
                            >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                      <div className="p-3 bg-black/40 border border-white/10 rounded-xl flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-[#E5B83B]">
                          <Car className="w-4 h-4" />
                        </div>
                        <div>
                          <div className="font-bold text-white">{assignedVehicle?.model || activeTrip.vehicleModel || 'Tesla Model Y'}</div>
                          <div className="text-[10px] font-mono text-[#E5B83B]">Skilt: {assignedVehicle?.licensePlate || activeTrip.vehicleLicensePlate || 'EP 17891'}</div>
                        </div>
                      </div>

                      <div className="p-3 bg-black/40 border border-white/10 rounded-xl flex items-center justify-between font-mono">
                        <div>
                          <span className="text-[9px] uppercase font-bold text-slate-400 block">Fastpris</span>
                          <span className="text-white font-bold">{activeTrip.estimatedPrice} NOK</span>
                        </div>
                        <div className="text-right">
                          <span className="text-[9px] uppercase font-bold text-emerald-400 block">Status</span>
                          <span className="text-emerald-300 font-bold capitalize">{activeTrip.status}</span>
                        </div>
                      </div>
                    </div>
